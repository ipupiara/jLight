...
uStInt evRecInTimeChecker(void)
{
	...
	if ((currentEvent->evType == evSecondsTick) && (storageBufferLow()))
	{
...
uStInt evRecTimeLowChecker(void)
{
	...
	if ((currentEvent->evType == evSecondsTick) && (storageBufferCritical()))
	{
...
uStInt evTestingChecker(void)
{
	...
	if (currentEvent->evType == evStoreButtonPressed)
	{
		...
		setStoreLight(on);
		syncStoreStorageBuffer(pCurrentStorageBuffer);
		setStoreLight(off);
